...
impl SymmetricMenu {
    pub fn exec_command(&self, session: &Session, authkey: &ObjectDescriptor) -> Result<(), MgmError> {
        loop {
            ...
            let res = match cmd.command {
                MgmCommandType::List => list_objects(&SymOps, session),
                MgmCommandType::GetKeyProperties => display_object_properties(&SymOps, session),
                MgmCommandType::Generate => generate_object(&SymOps, session, authkey, ObjectType::SymmetricKey),
                MgmCommandType::Import => self.import(session, authkey),
                MgmCommandType::Delete => delete_objects(&SymOps, session, &SymOps.get_all_objects(session)?),
    ...
    pub fn import(&self, session: &Session, authkey: &ObjectDescriptor) -> Result<(), MgmError> {
        ...
        key_data.push(YubihsmUi::get_aes_key_hex(&Cmdline, "Enter AES key in HEX format:")?);
        let key_algo = SymOps::get_symkey_algorithm_from_keylen(key_data[0].len())?;

        import_object(&SymOps, session, authkey, ObjectType::SymmetricKey, key_algo, key_data)
    }
