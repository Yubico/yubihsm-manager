...
pub fn exec_main_command(session: &Session, authkey: &ObjectDescriptor) -> Result<(), MgmError> {
    loop {
        ...
        let res = match cmd.command {
            MgmCommandType::GetRandom => get_random(session),
            MgmCommandType::BackupDevice => backup(session, authkey),
            MgmCommandType::RestoreDevice => restore(session, authkey),
            MgmCommandType::Reset => reset(session),
